...
import com.dthompson.timetap.R
...
object BLEConnectionManager {
    ...
    private val mServiceConnection = object : ServiceConnection {
        override fun onServiceConnected(componentName: ComponentName, service: IBinder) {
            ...
            Log.e(TAG, "Initialzing! initialize")
    ...
    fun connect(deviceAddress: String): Boolean {
        var result = false
        Log.d("connect","connection " + mBLEService.toString())
        if (mBLEService != null) {
            result = mBLEService!!.connect(deviceAddress)
            Log.d("connect","Not Null!")
...
//                if (gattService.getUuid().toString().equals(mContext.getString(R.string.char_uuid_serial),
//                                ignoreCase = true)) {
                    val gattCharacteristics = gattService.characteristics
                    for (gattCharacteristic in gattCharacteristics) {
                        Log.d("Characteristics", gattCharacteristic.uuid.toString())
                        uuid = if (gattCharacteristic.uuid != null) gattCharacteristic.uuid.toString() else ""

                        if (uuid.equals(mContext.resources.getString(R.string.char_uuid_serial),
                                        ignoreCase = true)) {
                            Log.d("Characteristics", "Got serial characteristic:" + gattCharacteristic.uuid.toString())
